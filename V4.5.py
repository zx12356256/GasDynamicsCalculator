...
class StagnationWindow:
    ...
    def create_conversion_tab(self):
        ...
        ttk.Radiobutton(input_frame, text="输入马赫数 Ma", variable=self.conversion_var, value="Ma").grid(row=0,
                                                                                                          column=0,
                                                                                                          padx=5,
                                                                                                          pady=5)
        ttk.Radiobutton(input_frame, text="输入速度系数 λ", variable=self.conversion_var, value="lambda").grid(row=0,
                                                                                                               column=1,
                                                                                                               padx=5,
                                                                                                               pady=5)
        ...
        # 添加显示图像的按钮
        btn_show_graphs = ttk.Button(self.conversion_frame, text="显示图象", command=self.show_graphs)
        btn_show_graphs.pack(pady=10)
        ...
        try:
            ...
            if self.conversion_var.get() == "Ma":
                ...
                for _ in range(10):  # 简单迭代
                    ...
                    df = math.sqrt((gamma + 1) / (2 + (gamma - 1) * Ma ** 2)) * (
                                1 - (gamma - 1) * Ma ** 2 / (2 + (gamma - 1) * Ma ** 2))
                    Ma = Ma - f / df
    ...
    def show_graphs(self):
        ...
        ma_values = np.linspace(0.0, 10.0, 400)
        ...
        # τ、π、ε-λ曲线
        axs[0, 1].plot(lambda_values, tau_values, label='τ', color='green')
        axs[0, 1].plot(lambda_values, pi_values, label='π', color='orange')
        axs[0, 1].plot(lambda_values, epsilon_values, label='ε', color='purple')
        ...
        # τ、π、ε-Ma曲线
        axs[1, 1].plot(ma_values, tau_values, label='τ', color='green')
        axs[1, 1].plot(ma_values, pi_values, label='π', color='orange')
        axs[1, 1].plot(ma_values, epsilon_values, label='ε', color='purple')
