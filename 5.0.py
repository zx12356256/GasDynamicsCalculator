...
class FlowWindow:
    ...
    # ===== Rayleigh =====
    # def rayleigh_Tt(self, M: float, T: float) -> float:
    #     """总温"""
    #     return T * (1 + self.k * M**2) / (1 + (self.k - 1) / 2 * M**2)

    # def solve_rayleigh_exit_M(self, M1: float, T01: float, T02: float,
    #                           tol: float = 1e-6) -> float:
    #     """Rayleigh 管出口 M"""
    #     low, high = (1e-6, 1 - 1e-6) if M1 < 1 else (1 + 1e-6, 10)
    #     f_low = self.rayleigh_Tt(low, T02) - self.rayleigh_Tt(M1, T01)
    #     f_high = self.rayleigh_Tt(high, T02) - self.rayleigh_Tt(M1, T01)
    #     if f_low * f_high > 0:
    #         raise RuntimeError("根不在区间内")
    #     for _ in range(100):
    #         mid = 0.5 * (low + high)
    #         f_mid = self.rayleigh_Tt(mid, T02) - self.rayleigh_Tt(M1, T01)
    #         if abs(f_mid) < tol:
    #             return mid
    #         if f_mid * f_low < 0:
    ...
    #     self.ray_res = {
    #         "出口马赫数": ttk.Label(resf, text="出口马赫数 = "),
    #         "出口总温": ttk.Label(resf, text="出口总温 = "),
    #         "总压比 P02/P01": ttk.Label(resf, text="总压比 P02/P01 = "),
    #     }
    ...
    #         q = float(self.ray_entry["q"].get()) * 1e3  # kJ/kg→J/kg
    #     except ValueError:
    #         messagebox.showerror("输入错误", "所有输入必须为数字!")
    #         return

    #     if T01 <= 0:
    #         messagebox.showerror("输入错误", "T01 必须 > 0")
    #         return
    ...
    #         T02 = T01 + q / Cp
    #         if T02 <= 0:
    #             raise ValueError("加热量过大, 导致 T02 ≤ 0")

    #         M2 = self.solve_rayleigh_exit_M(M1, T01, T02)

    #         P02_P01 = ((1 + self.k * M1**2) / (1 + self.k * M2**2)) * \
    #                   ((1 + (self.k - 1) / 2 * M2**2) /
    #                    (1 + (self.k - 1) / 2 * M1**2)) ** (self.k / (self.k - 1))

    #         self.ray_res["出口马赫数"].config(text=f"出口马赫数 = {M2:.4f}")
    #         self.ray_res["出口总温"].config(text=f"出口总温 (K) = {T02:.2f}")
    #         self.ray_res["总压比 P02/P01"].config(text=f"P02/P01 = {P02_P01:.4f}")
    #     except Exception as err:
    #         messagebox.showerror("计算错误", str(err))
    ...
    #     self.mass_entry: dict[str, ttk.Entry] = {}
    #     for i, (txt, key) in enumerate([
    #         ("主流质量流率 ṁ1 (kg/s):", "m1"),
    #         ("主流速度 V1 (m/s):", "V1"),
    #         ("附加质量 Δṁ (kg/s):", "dm"),
    ...
    #         e = ttk.Entry(lf)
    #         e.grid(row=i, column=1, sticky="w", padx=4, pady=4)
    #         self.mass_entry[key] = e

    #     ttk.Button(f, text="计算", command=self._calc_mass).pack(pady=6)

    #     resf = ttk.LabelFrame(f, text="计算结果")
    #     resf.pack(fill="x", padx=10, pady=6)
    #     self.mass_res = {
    #         "出口质量流率": ttk.Label(resf, text="出口质量流率 = "),
    #         "出口流速": ttk.Label(resf, text="出口流速 = "),
    #         "动量变化率": ttk.Label(resf, text="动量变化率 = "),
    #     }
    #     for i, lbl in enumerate(self.mass_res.values()):
    #         lbl.grid(row=i, column=0, sticky="w", padx=6, pady=4)

    # def _calc_mass(self) -> None:
    #     """变质量流计算"""
    #     try:
    #         m1 = float(self.mass_entry["m1"].get())
    #         V1 = float(self.mass_entry["V1"].get())
    #         dm = float(self.mass_entry["dm"].get())
    #         Vadd = float(self.mass_entry["Vadd"].get())
    #     except ValueError:
    #         messagebox.showerror("输入错误", "所有输入必须为数字!")
    #         return

    #     if m1 <= 0:
    #         messagebox.showerror("输入错误", "主流质量流率必须 > 0")
    #         return

    #     try:
    #         m2 = m1 + dm
    #         if m2 <= 0:
    #             raise ValueError("总体质量流率 ≤ 0")
    #         V2 = (m1 * V1 + dm * Vadd) / m2
    #         dP = m2 * V2 - m1 * V1

    #         self.mass_res["出口质量流率"].config(text=f"出口质量流率 (kg/s) = {m2:.3f}")
    #         self.mass_res["出口流速"].config(text=f"出口流速 (m/s) = {V2:.2f}")
    #         self.mass_res["动量变化率"].config(text=f"动量变化率 (N) = {dP:.2f}")
    #     except Exception as err:
    #         messagebox.showerror("计算错误", str(err))
