...
class MainWindow:
    ...
    def open_stagnation(self):
        ...
        stagnation_window.master.geometry("500x400+200+200")  # 左边窗口的位置
    ...
    def calculate(self):
        try:
            ...
            if mode == "已知 Ma1+θ → 求膨胀后参数":
                ...
                Ma1 = float(self.entry_ma1.get())
                Ma2_input = float(self.entry_ma2.get())
                theta = float(self.entry_theta.get())

                if Ma1 < 1 or Ma2_input < 1:
                    messagebox.showerror("输入错误", "波前和波后马赫数 Ma1, Ma2 应该 ≥ 1")
                    return

                nu1 = self.prandtl_meyer(Ma1, gamma)
                nu2 = nu1 + theta
                Ma2_calc = self.inverse_prandtl_meyer(nu2, gamma)

                error = abs(Ma2_input - Ma2_calc)
                self.labels['result'].config(
                    text=f"理论计算的反射后马赫数 Ma2 ≈ {Ma2_calc:.4f}，输入 Ma2 = {Ma2_input:.4f}，误差 = {error:.4e}"
                )
                self.labels['p_ratio'].config(text="")
                self.labels['t_ratio'].config(text="")
                self.labels['pm_angle'].config(text=f"ν1 = {nu1:.2f}°, ν2理论 = {nu2:.2f}°")
                return
