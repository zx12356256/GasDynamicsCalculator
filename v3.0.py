...
class MainWindow:
    ...
    def open_stagnation(self):
        ...
        stagnation_window.master.geometry("650x500+215+205")  # 左边窗口的位置
...
class ExpansionWindow:
    def __init__(self, master):
        ...
        # 输入控件
        ttk.Label(input_frame, text="初始马赫数 Ma1:").grid(row=0, column=0, padx=5, pady=5)
        self.entry_ma1 = ttk.Entry(input_frame)
        self.entry_ma1.grid(row=0, column=1, padx=5, pady=5)
        ...
        ttk.Label(input_frame, text="转折角 θ (°):").grid(row=2, column=0, padx=5, pady=5)
        self.entry_theta = ttk.Entry(input_frame)
        self.entry_theta.grid(row=2, column=1, padx=5, pady=5)
        ...
        self.labels = {
            'ma2': ttk.Label(result_frame, text="膨胀后马赫数 Ma2 = "),
    ...
    def calculate(self):
        try:
            ...
            if Ma1 < 1:
                messagebox.showerror("错误", "初始马赫数必须大于1")
                return
            ...
            # 更新结果
            self.labels['ma2'].config(text=f"膨胀后马赫数 Ma2 = {Ma2:.4f}")
            self.labels['p_ratio'].config(text=f"压力比 P2/P1 = {p_ratio:.4f}")
            self.labels['t_ratio'].config(text=f"温度比 T2/T1 = {t_ratio:.4f}")
            self.labels['pm_angle'].config(text=f"普朗特-迈耶角 ν = {nu2:.2f}°")
